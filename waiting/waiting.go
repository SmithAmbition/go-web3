...
func NewWaitBlockHeight(web *web3Manager.Web3Manager, height uint64) *WaitBlockHeight {
	w := &WaitBlockHeight{
...
func (w *WaitBlockHeight) Waiting() {
	w.WaitingFn(func() {
		bm, err := w.Web3.Eth.GetBlockNumber()
		if err == nil {
...
func NewWaitTxReceipt(web *web3Manager.Web3Manager, txHash string) *WaitTxReceipt {
	w := &WaitTxReceipt{
		Web3Waiting{Web3: web},
		txHash, nil,
	}
...
func (w *WaitTxReceipt) Waiting() {
	w.WaitingFn(func() {
		receipt, err := w.Web3.Eth.GetTransactionReceipt(w.txhash)
		if err == nil {
...
func NewWaitTxReceiptAry(web *web3Manager.Web3Manager, txHashAry []string) *WaitTxReceiptAry {
	wt := &WaitTxReceiptAry{
		Web3Waiting{Web3: web},
...
func (wt *WaitTxReceiptAry) Waiting() {
	wt.WaitingFn(func() {
		...
		for key, item := range wt.Receipts {
			...
			receipt, err := wt.Web3.Eth.GetTransactionReceipt(key)
